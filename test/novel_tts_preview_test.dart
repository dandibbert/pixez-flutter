...
void main() {
  ...
  test('voice/text GET preserves exact parameters and plays the saved response', () async {
    ...
    expect(audio.loadedPath, startsWith('${temporary.path}${Platform.pathSeparator}'));
    expect(audio.loadedPath, endsWith('${Platform.pathSeparator}preview.mp3'));
  ...
  test('arbitrary custom variables reach the server without injected fields', () async {
    final custom = settings(
      template: 'http://127.0.0.1:${server.port}/tts'
          '?speaker_id={speaker_id}&style={style_name}&text={text}',
    ).copyWith(customVariables: {
      'speaker_id': '428',
  ...
  for (final status in [401, 500]) {
    test('HTTP $status retains status and response details as a synthesis error', () async {
      final body = status == 401 ? 'Missing speech API key' : 'Speech worker unavailable';
      respond = (request) async {
      ...
      await expectLater(
        ...
        throwsA(isA<NovelTtsPreviewException>()
            .having((error) => error.stage, 'stage', NovelTtsPreviewStage.synthesis)
            .having((error) => error.cause.toString(), 'HTTP status', contains('HTTP $status'))
            .having((error) => error.cause.toString(), 'response body', contains(body))),
      );
  ...
  test('an unusable temporary directory is reported at savingAudio', () async {
    final obstruction = await File('${temporary.path}/not_a_directory')
    ...
    await expectLater(
      play(settings()).timeout(_deadline),
      throwsA(isA<NovelTtsPreviewException>()
          .having((error) => error.stage, 'stage', NovelTtsPreviewStage.savingAudio)
          .having((error) => error.cause, 'filesystem cause', isA<FileSystemException>())),
    );
    expect(requests, hasLength(1), reason: 'The API succeeds before saving fails');
    expect(playerCreations, 0);
    expect(await obstruction.readAsString(), 'existing file');
  });

  test('a decoder failure is reported at loadingAudio and removes the saved file', () async {
    ...
    await expectLater(
      play(settings()).timeout(_deadline),
      throwsA(isA<NovelTtsPreviewException>()
          .having((error) => error.stage, 'stage', NovelTtsPreviewStage.loadingAudio)
          .having((error) => error.cause, 'original decoder failure', same(failure))),
    );
    expect(audio.bytesReadFromFile, _audioBytes);
  ...
  test('a playback failure keeps its stage and removes temporary audio', () async {
    ...
    await expectLater(
      play(settings()).timeout(_deadline),
      throwsA(isA<NovelTtsPreviewException>()
          .having((error) => error.stage, 'stage', NovelTtsPreviewStage.playback)
          .having((error) => error.cause, 'original playback failure', same(failure))),
    );
    expect(audio.playCount, 1);
    expect(audio.disposeCount, 1);
    expect(await temporary.list().toList(), isEmpty);
  ...
  test('stop aborts an outstanding real HTTP request before creating a player', () async {
  ...
  test('stop while loading audio prevents playback and cleans the saved file', () async {
  ...
  test('dispose during playback stops the player and removes temporary audio', () async {
    ...
    expect(requests, hasLength(1), reason: 'Disposed previews cannot restart');
  });

  test('a cleanup failure does not replace the original decoder error', () async {
  ...
  test('a standalone cleanup failure is reported as cleanup', () async {
    ...
    await expectLater(
      play(settings()).timeout(_deadline),
      throwsA(isA<NovelTtsPreviewException>()
          .having((error) => error.stage, 'stage', NovelTtsPreviewStage.cleanup)
